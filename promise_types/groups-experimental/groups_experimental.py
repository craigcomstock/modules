...
class GroupsExperimentalPromiseTypeModule(PromiseModule):
    ...
    def validate_promise(self, promiser, attributes):
        ...
        if self._name_regex.match(promiser) is None:
            self.log_warning(
                f"Promiser groupname '{promiser}' should match regular expression '[a-z_][a-z0-9_-]*[$]?'"
            )

        # check promiser length not too long
        if len(promiser) > self._name_maxlen:
            raise ValidationError(
                f"Promiser '{promiser}' is too long: ({len(promiser)} > {self._name_maxlen})"
            )
        ...
        if "policy" in attributes:
            ...
            if policy not in ("present", "absent"):
                raise ValidationError(
                    f"Invalid value '{policy}' for attribute policy: must be 'present' or 'absent'"
                )
            ...
            if policy == "absent":
                ...
                if "members" in attributes:
                    self.log_warning(
                        f"Cannot assign members to absent group '{promiser}'"
                    )
        ...
        if "gid" in attributes:
            ...
            if type(gid) == str:
                try:
                    ...
                    raise ValidationError(
                        f"Invalid value '{gid}' for attribute gid: expected integer literal"
                    )
        ...
        if "members" in attributes:
            ...
            for attr in members:
                if attr not in ("only", "include", "exclude"):
                    raise ValidationError(
                        f"Invalid value '{attr}' in attribute members: must be 'only', 'exclude' or 'include'"
                    )
            ...
            if "include" in members and "exclude" in members:
                ...
                if duplicates != set():
                    raise ValidationError(
                        f"Users {duplicates} both included and excluded from group '{promiser}'"
                    )
    ...
    def evaluate_promise(self, promiser, attributes):
        ...
        if group is not None:
            ...
            if promised_gid is not None and promised_gid != group.gid:
                self.log_error(
                    f"There is an existing group '{promiser}' with a different GID ({group.gid}) than promised ({promised_gid})"
                )
        ...
        self.log_debug(
            f"'{repairs}' repairs and '{failed_repairs}' failed repairs to promiser '{promiser}'"
        )
        if failed_repairs > 0:
    ...
    def _include_users(self, group, users):
        ...
        for user in users:
            ...
            if user in group.members:
                self.log_debug(
                    f"User '{user}' already included in group '{group.name}'"
                )
            else:
                ...
                try:
                    ...
                    self.log_error(
                        f"Failed to add user '{user}' to group '{group.name}': {e}"
                    )
                    failed_repairs += 1
                else:
    ...
    def _exclude_users(self, group, users):
        ...
        for user in users:
            self.log_debug(
                f"User '{user}' should be excluded from group '{group.name}'"
            )
            if user in group.members:
                ...
                try:
                    ...
                    self.log_error(
                        f"Failed to remove user '{user}' from group '{group.name}': {e}"
                    )
                    failed_repairs += 1
                else:
                ...
                self.log_debug(
                    f"User '{user}' already excluded from group '{group.name}'"
                )
    ...
    def _only_users(self, group, users):
        ...
        if set(users) != set(group.members):
            self.log_debug(
                f"Group '{group.name}' does not only contain members {users}"
            )
            try:
                group.set_members(users)
            except GroupException as e:
                self.log_error(
                    f"Failed to set members of group '{group.name}' to only users {users}: {e}"
                )
                failed_repairs += 1
            else:
                self.log_info(
                    f"Members of group '{group.name}' set to only users {users}"
                )
                repairs += 1
        else:
